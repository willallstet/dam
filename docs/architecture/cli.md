...
- [ADR-058 — API keys with scopes for headless CLI use](../adrs/058-api-keys-headless-auth.md) — the `dam auth token` sub-tree; `DAM_TOKEN` now accepts API keys (`pk_…` prefix) in the same Bearer slot the JWT flow already uses.
