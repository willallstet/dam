...
- **Resolver surface** — `AgentResolver` is exported from the `agent` module's `index.ts`. Downstream verbs (`dam chat`, `dam import`, …) import it from there and ask the module's compose for an `AgentService` bound to the resolved Active Host.
...
- **Wire shape** — POST `<server>/api/agents/<id>/import`, multipart/form-data with one part `bundle: application/gzip`. Bearer auth. Same contract the UI's [`importBundle`](../../packages/ui/src/modules/files/api/import-bundle.ts) targets; the server-side design rationale lives in [ADR-044](../adrs/044-file-import.md).
